...
#define SOFTWARE_I2C   //HARDWARE_I2C
...
MPUHandle MyMpu = {.MPUIntPIN = GPIO_PIN_1,.pitch = 0,.row = 0,.yaw = 0};
...
uint8_t MPU6050_Write_Len(uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf)
{
    ...
    HAL_I2C_Master_Transmit(&hi2c2,addr<<1,TempBuff,len+1,999);
#endif
#ifdef SOFTWARE_I2C
    uint8_t i;
    MyI2C_Start();
...
u8 MPU_Write_Byte(u8 reg,u8 data)
{
    ...
    TempBuff[0] = reg;
    TempBuff[1] = data;
    HAL_I2C_Master_Transmit(&hi2c2,(MPU_ADDR<<1)|0,TempBuff,2,999);
#endif
#ifdef SOFTWARE_I2C
    MyI2C_Start();
...
uint8_t MPU6050_ReadReg(uint8_t RegAddress)
{
    uint8_t Data;
#ifdef HARDWARE_I2C
    HAL_I2C_Master_Transmit(&hi2c2,(MPU6050_Address<<1)|0x00,&RegAddress,1,999);//写模式发送寄存器地址
    HAL_I2C_Master_Receive(&hi2c2,(MPU6050_Address<<1)|0x01,&Data,1,999); //读模式读取数据
#endif
...
uint8_t MPU6050_Read_Len(uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf)
{
    ...
    HAL_I2C_Master_Transmit(&hi2c2,(addr<<1)|0x00,&reg,1,999); //写模式发送寄存器地址
    HAL_I2C_Master_Receive(&hi2c2,(addr<<1)|0x01,buf,len,999); //读模式读取数据
#endif
