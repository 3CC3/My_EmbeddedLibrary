 ...
 * @brief 按下按键后要做的任务
 */
void PressKey2DoTask(MenuHandle *MENU,TaskLinkList *L)
{
    if(hKey.KeyLooseFlag == 1 || hKey2.KeyLooseFlag == 1 || hKey3.KeyLooseFlag == 1)
    {   /*What to do when you press the key*/

        //MenuSwitch(&TestU8G2,MENU);
        ReMenuSwitch(&TestU8G2,MENU,L,hKey.KeyLooseFlag,hKey2.KeyLooseFlag,hKey3.KeyLooseFlag);
        /*----------------------------------*/
        hKey.KeyLooseFlag = 0;
