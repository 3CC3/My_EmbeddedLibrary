...
uint8_t parse_json_msg(uint8_t *json_msg,uint8_t json_len)
{
    ...
    char * value;
    size_t valueLength;
    //1.确定解析缓冲区是否包含有效的JSON文档
    result = JSON_Validate((const char *)json_msg, json_len);
    if( result == JSONSuccess)
    {
        //2.在JSON文档中查找一个键或者数组索引，并把指针outValue指向查找到的键的值的地址
