...
ESPMQTT_Status ESPMQTT_Start(void)
{
    ...
    /*0.关闭回显*/
    OLED_ShowString(4,1,"0.Close Echo    ");
    ...
    /*1.设置WiFi模式*/
    OLED_ShowString(4,1,"1.Set WiFi Mode ");
    ...
    /*2.上电不自动连接AP(WiFi)*/
    OLED_ShowString(4,1,"2.AUTOConnectCFG");
    ...
    /*3.重启ESP32*/
    OLED_ShowString(4,1,"3.Rest ESP      ");
    ...
    /*4.连接WiFi*/
    OLED_ShowString(4,1,"4.Connect WiFi  ");
    ...
    /*5.配置MQTT用户属性*/
    OLED_ShowString(4,1,"5.MQTT UserCFG  ");
    ...
    /*6.连接MQTT Broker*/
    OLED_ShowString(4,1,"6.MQTTContBroker");
    ...
    OLED_ShowString(4,1,"ESPMQTTInitSucs!");
}
...
ESPMQTT_Status JSONBreathLEDTask(const char *Data,int DataLength)
{
    uint8_t LEDCCR = 0;
    JSONStatus_t Status;
    const char query[] = "params.PWMCCR";
    size_t QueryLength = strlen(query);
    size_t OutValueLength;
    char *OutValue = NULL;
    Status = JSON_Validate(Data,DataLength);
...
ESPMQTT_Status JSONLEDRibTask(const char *Data,int DataLength)
{
    uint8_t LEDMode = 0;
    JSONStatus_t Status;
    const char query[] = "params.sw";
    size_t QueryLength = strlen(query);
    size_t OutValueLength;
    char *OutValue = NULL;
    Status = JSON_Validate(Data,DataLength);
    ...
    if(Status == JSONSuccess)
    {
        ...
        if(Status == JSONSuccess)
        {
            ...
            LEDMode = ConvertString2Num(OutValue);
            printf("LED Mode:%d\r\n",LEDMode);


            //__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_1,LEDMode);
            hLedRib.RuningMode = LEDMode;
