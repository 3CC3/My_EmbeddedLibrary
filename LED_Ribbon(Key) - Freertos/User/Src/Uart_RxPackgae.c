...
{
    uint8_t tempbuff[10],count = 0;
    static uint8_t TouchNum = 0;
    if(Uart_PackageHandle->RxEndFalg == 1)
    {
        ...
        /*  帧头:'@'  帧尾:'$'   */
//        if(Uart_PackageHandle->Rx_Package[0] == '@' && Uart_PackageHandle->Rx_Package[Uart_PackageHandle->Cur_RxLength-1] == '$')
//        {
//            while(Uart_PackageHandle->Rx_Package[count+1] != '$')
//            {
//                tempbuff[count] = Uart_PackageHandle->Rx_Package[count+1];
//                count++;
//            }
//            tempbuff[count] = '\0';
//            if(!strcmp((char*)tempbuff,"Touch"))
//            {
//                TouchNum++;
//                if(TouchNum >= 4) TouchNum=0;
//                hLedRib.RuningMode = TouchNum;
//                OLED_ShowNum(4,10,TouchNum,1);
//            };
//        }
...
uint8_t Uart_ReceiveEsp(UartRx_Package *Uart_PackageHandle,const char *Receive)
{
    uint8_t ret = ERROR;
    if(Uart_PackageHandle->RxEndFalg == 1)
    {
        //printf("Receive:%s\r\n",Uart_PackageHandle->Rx_Package);
        if(strstr((char *)Uart_PackageHandle->Rx_Package,Receive) != NULL)
        {
            ret = SUCCESS;
        }
        else ret = ERROR;
